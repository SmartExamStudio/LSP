...
public class FormLaporanAdmin extends JFrame {
    ...
    private void initComponents() {
        ...
        setSize(1100, 700); // Ukuran lebih proporsional
        setMinimumSize(new Dimension(900, 600)); // Ukuran minimum
    ...
    private JPanel createRingkasanTab() {
        ...
        // Filter Panel (Top) - lebih compact
        JPanel filterPanel = createFilterPanel();
        panel.add(filterPanel, BorderLayout.NORTH);

        // Main Content dengan JSplitPane untuk proporsi yang lebih baik
        JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
        splitPane.setBackground(new Color(40, 40, 45));
        splitPane.setBorder(null);
        splitPane.setDividerSize(5);
        splitPane.setResizeWeight(0.35); // 35% untuk ringkasan, 65% untuk tabel

        // Ringkasan Panel (Top - lebih kecil)
        JPanel ringkasanPanel = createRingkasanPanel();
        splitPane.setTopComponent(ringkasanPanel);

        // Table Panel (Bottom - lebih besar)
    ...
    private JPanel createFilterPanel() {
        ...
        // Button Generate
        btnGenerate = createStyledButton("Filter", new Color(52, 152, 219));
    ...
    private JPanel createRingkasanPanel() {
        ...
        panel.setBorder(BorderFactory.createCompoundBorder(
                ...
                BorderFactory.createEmptyBorder(10, 15, 10, 15) // Padding lebih kecil
        ));

        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 10, 5, 10); // Insets lebih kecil
        gbc.fill = GridBagConstraints.HORIZONTAL;

        // Row 1: 3 Cards utama + Total Transaksi (4 cards dalam 1 baris)
        gbc.gridx = 0;
        gbc.gridy = 0;
        gbc.weightx = 1.0;
        ...
        // Row 2: Info Tambahan (semua dalam 1 baris)
        gbc.gridx = 0;
        gbc.gridy = 1;
        gbc.gridwidth = 2;
        ...
        JLabel lblCheckbox = new JLabel("☐ Info Tambahan");
        lblCheckbox.setForeground(new Color(180, 180, 180));
        lblCheckbox.setFont(new Font("Arial", Font.PLAIN, 11));
        infoPanel.add(lblCheckbox);

        JLabel lblInfo = new JLabel("  Total Ongkir: Rp 360.000   |   ");
        ...
        lblInfoTambahan.setFont(new Font("Arial", Font.ITALIC, 10));
        infoPanel.add(lblInfoTambahan);

        panel.add(infoPanel, gbc);

        // Row 2: Total Kas Masuk (di kanan)
        gbc.gridx = 2;
        gbc.gridwidth = 2;
        JPanel kasPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 5, 0));
        ...
        lblKasLabel.setFont(new Font("Arial", Font.BOLD, 11));
        kasPanel.add(lblKasLabel);

        lblTotalKesMasuk = new JLabel("Rp 0");
        lblTotalKesMasuk.setForeground(new Color(46, 204, 113));
        lblTotalKesMasuk.setFont(new Font("Arial", Font.BOLD, 12));
    ...
    private JPanel createCardRingkasan(String title, String value, Color color, JLabel valueLabel) {
        ...
        card.setPreferredSize(new Dimension(200, 70)); // Ukuran lebih kecil
        card.setMinimumSize(new Dimension(150, 60));
        card.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(color, 2),
                BorderFactory.createEmptyBorder(8, 10, 8, 10) // Padding lebih kecil
        ));

        JLabel lblTitle = new JLabel(title);
        lblTitle.setFont(new Font("Arial", Font.PLAIN, 11)); // Font lebih kecil
        lblTitle.setForeground(new Color(200, 200, 200));

        valueLabel.setText(value);
        valueLabel.setFont(new Font("Arial", Font.BOLD, 18)); // Font value lebih kecil
    ...
    private void loadLaporanData() {
        ...
        System.out.println("Periode: " + from + " - " + to);
        ...
        if ((Boolean) result.get("success")) {
            ...
            System.out.println("Total hasil: " + transaksiList.size());
            ...
            if (transaksiList.isEmpty()) {
                JOptionPane.showMessageDialog(this,
                        "Tidak ada transaksi ditemukan dengan filter:\n" +
                                "- Pembayaran: " + pembayaran + "\n" +
                                "- Channel: " + channel + "\n" +
                                "- Periode: " + new SimpleDateFormat("dd/MM/yyyy").format(from) + " s/d " +
